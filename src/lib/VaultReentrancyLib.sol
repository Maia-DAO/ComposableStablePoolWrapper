...
library VaultReentrancyLib {
    ...
    function ensureNotInVaultContext(address vault) internal view {
        ...
        (, bytes memory revertData) =
            vault.staticcall{gas: 10_000}(abi.encodeWithSelector(IVault.manageUserBalance.selector, 0));

        // If the Vault is being reentered, the revertData will be non-empty, and we should revert.
        // If the Vault is not being reentered, the revertData will be empty, and we should not revert.
        if (revertData.length > 0) revert Reentracy();
    }

    /// @notice Reverts if the Vault is currently in a transaction.
    error Reentracy();
}
